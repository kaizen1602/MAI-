...
-   `GET /api/users/{user_id}`
    ...
    -   **Método:** `GET`
    -   **Acceso:** Requiere Autenticación.
    -   **Detalles:** Es crucial que este endpoint **NO** devuelva información sensible como `email`, `adress_details`, `is_verified`. Solo debería devolver `user_id`, `name`, `created_at` y quizás una calificación promedio calculada a partir de las `reviews`.
